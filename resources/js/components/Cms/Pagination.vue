        ...
        <div class="card lg:flex w-full">
            <nav class="flex flex-wrap p-5" v-if="links.length > 3">
                <template v-for="(link, p) in links" :key="p">
                    <div v-if="link.url === null" class="btn text-xs rounded-full mr-2 mb-2 lg:mb-0 text-gray-300 hover:text-gray-300 border-gray-300 bg-transparent cursor-default"
                    v-html="link.label" />
                    <Link v-else :href="link.url" class="btn text-xs rounded-full mr-2 mb-2 lg:mb-0" :class="{'btn-admin': link.active, 'btn-outlined btn-admin-secondary': !link.active}" v-html="link.label" />
                </template>
            </nav>
            <div
                class="flex items-center ml-auto p-5 border-t text-xs lg:border-t-0 border-gray-200 dark:border-gray-900"
            >
